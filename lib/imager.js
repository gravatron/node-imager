...
Imager.prototype = {
  ...
  upload: function (files, callback, variant, nameOverride) {
    var self = this;
    var getFileInfo = function (file, cb) {
      var f = {
        size: fs.statSync(file).size,
        type: mime.lookup(file),
        name: nameOverride || file.split('/')[file.split('/').length - 1],
        path: file
      };
    ...
    var prepare = function (file, fn) {
      var filename = nameOverride || Math.round(new Date().getTime());
  ...
  prepareUpload: function (file, filename, variant, fn) {
    ...
    if (variants.cropAndResize) {
      Object.keys(variants.cropAndResize).forEach(function (name) {
        var processFiles = function (cb) {
          var preset = {
            name: name,
            type: variants.cropAndResize[name]
          };
          self.cropAndResizeFile(file, preset, filename, cb);
        };
        asyncArr.push(processFiles);
      });
    }
    ...
    if (variants.full) {
      Object.keys(variants.full).forEach(function (name) {
        var processFiles = function (cb) {
          var preset = {
            name: name,
            type: variants.full[name]
          };
          self.fullFile(file, preset, filename, cb);
        };
        asyncArr.push(processFiles);
      });
    }
  ...
  /**
   *  Copy file at path src to file path dest
   */
  copy: function(fpathSrc, fpathDest, cb) {
    var fSrc = fs.createReadStream(fpathSrc);
    var fDest = fs.createWriteStream(fpathDest);
    fSrc.on('end', cb);
    fSrc.pipe(fDest);
  },
  ...
  fullFile: function (file, preset, fname, cb) {
    var ct = {
      'image/jpeg': '.jpg',
      'image/png': '.png',
      'image/gif': '.gif'
    ...
    this.copy(file.path, dest, function() {
  ...
  cropFile: function (file, preset, fname, cb) {
    ...
    gm(file['path'])
      .autoOrient()
      .crop(preset.size.split('x')[0], preset.size.split('x')[1], preset.type.crop.split('x')[2], preset.type.crop.split('x')[3])
      .write(dest, function (err) {
        if (err) return cb(err);
        self['pushTo' + self.storage](dest, remoteFile, filename, file['type'], cb);
      });
  ...
  cropAndResizeFile: function (file, preset, fname, cb) {
    var ct = {
      'image/jpeg': '.jpg',
      'image/png': '.png',
      'image/gif': '.gif'
    ...
    gm(file['path'])
      .autoOrient()
      .crop(preset.type.crop.split('x')[0], preset.type.crop.split('x')[1], preset.type.crop.split('x')[2], preset.type.crop.split('x')[3])
      .gravity('Center')
  ...
  prepareRemove: function (file, fn, variant) {
    ...
    if (variants.cropAndResize) {
      Object.keys(variants.cropAndResize).forEach(function (preset) {
        var removeFiles = function (cb) {
          self['removeFrom' + self.storage](file, preset, cb);
        };
        asyncArr.push(removeFiles);
